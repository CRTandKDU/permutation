...
The "mad dancing" variant introduces one new binary relational constraint
between rows in the links matrix. Some couples of primary columns should 
not only be covered (existence constraint) but by rows attaching the same 
value, an integer, to each column.

This variant comes from the "mad" series of puzzles:
http://mosa.ique.free.fr/jeux/jeufou.htm
where n^2 blocks with colored half-figures on each edge must be placed on
n^2 cells (i) matching proper left and right half-figures; and (ii) matching
colors on each inner edges.
...
class DLX:
    ...
    def getrow( self, row, rowidentifiers ):
        i = row
        while True:
#            print "getrow", i
            try:
                id = rowidentifiers.index(i)
#                print "-----"
                return self.Nrows[id]
            except ValueError:
                i = self.R[i]
                if i == row:
                    break
    ...
    def solve(self,
        ...
        for sol in self._solve(0, columnselector, columnselectoruserdata, valueconstraint, self.statistics):
            yield sol


    def _solve(self, depth, columnselector, columnselectoruserdata, valueconstraint, statistics):
        ...
        r = self.D[c]
        while r != c:
            if (None == valueconstraint) or (None != valueconstraint and valueconstraint( r, self.partialsolution ) ):
                ...
                # Recursively search.
                for sol in self._solve(depth+1, columnselector, columnselectoruserdata, valueconstraint, statistics):
                    yield sol
